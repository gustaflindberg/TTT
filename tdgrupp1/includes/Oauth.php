...
define('CONSUMER_KEY', 'lvgBfiCvMRGgNTbF5zM8kxN14');
define('CONSUMER_SECRET', '4TjIqUy95yrfBgsb8xqrcmDDmnev78fGHIva62hlGQMKlOVZTw');
/**
...
function invalidate_bearer_token($bearer_token){
	...
	$retrievedhtml = curl_exec ($ch); // execute the curl
	curl_close($ch); // close the curl
    //$retrievedhtml = html_entity_decode(json_encode($retrievedhtml));
...
function search_for_a_term($bearer_token, $linea, $result_type='mixed', $count='15'){
	...
	$headers = array( 
		"GET /1.1/trends/place.json".$formed_url." HTTP/1.1", 
		"Host: api.twitter.com", 
		"User-Agent: jonhurlock Twitter Application-only OAuth App v.1",
